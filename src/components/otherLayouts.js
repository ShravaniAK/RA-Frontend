...
//    handleHelp();
  ...
  const languages = ['C++', 'Java', 'JavaScript', 'Go', 'Ruby', 'Python'];
...
<Grid container spacing={2} style={{ padding: '10px', marginTop: '10px' }}>
      {/* Render form components for each language */}
      <Grid item xs={12}>
        <Paper elevation={3} style={{ padding: 6, minHeight: 70 }}>
          <Typography variant="subtitle2">Language Name: {props.selectedLanguage}</Typography>
          {/* Add your form components here for level, duration, time, last_used */}
          {/* Example: Level */}
          <FormControl sx={{ m: 1, minWidth: 170 }}>
            <InputLabel id={`select-level-${props.selectedLanguage}`}>Level</InputLabel>
            <Select
              labelId={`select-level-${props.selectedLanguage}`}
              id={`select-level-${props.selectedLanguage}`}
              value={props.level}
              label="Level"
